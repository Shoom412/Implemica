...
public class ArrayData {
    ...
    public ArrayData(int vertices){			//Argument is the number of vertices in the graph

        Cost = new int [vertices][vertices];
    }

        public void setEdge (int i, int j, int weight){			//Set the weight between the neighbors
        Cost [i][j] = weight;
    }

    public int getCost(int i , int j){			/* Returns 0 if I and J are the same, a large number of returns 
    												if there is no connection between the tops of the ribs or weight*/
        if (i==j){
            return 0;
        }
        if (Cost[i][j]==0){
    ...
    public Integer[] distancesFrom (int source){			//Return an array of the distances from source to each other vertex
